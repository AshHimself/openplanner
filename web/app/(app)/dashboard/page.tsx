    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-5">
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">{totalAllocated}h allocated of {totalCapacity}h</p>
          </CardContent>
        </Card>
        ...
        <Card className={conflicts.length > 0 ? "border-red-300" : ""}>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            ...
            <AlertTriangle className={`h-4 w-4 ${conflicts.length > 0 ? "text-red-500" : "text-muted-foreground"}`} />
          </CardHeader>
          <CardContent>
        ...
        <Card className={overBudgetCount > 0 ? "border-red-300" : ""}>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            ...
            <Wallet className={`h-4 w-4 ${overBudgetCount > 0 ? "text-red-500" : "text-muted-foreground"}`} />
          </CardHeader>
          <CardContent>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
              ...
              <div key={project.id} className="space-y-1">
                <div className="flex items-center justify-between text-sm">
                  <span className="flex items-center gap-2">
                    <span className="inline-block h-2.5 w-2.5 rounded-sm" style={{ backgroundColor: project.color }} />
                    {project.name}
                  </span>
                ...
                <div className="h-2 overflow-hidden rounded-full bg-muted">
                  <div className="h-full rounded-full" style={{ width: `${(hours / maxHours) * 100}%`, backgroundColor: project.color }} />
                </div>
              </div>
            ))}
                ...
                <div key={project.id} className="space-y-1">
                  <div className="flex items-center justify-between text-sm">
                    ...
                    {budget ? (
                      ...
                      <span className="tabular-nums text-muted-foreground">{fmtCurrency(forecast)} / no budget</span>
                    )}
                  </div>
                  <div className="h-2 overflow-hidden rounded-full bg-muted">
                    <div className={`h-full rounded-full ${over ? "bg-red-500" : "bg-emerald-500"}`} style={{ width: budget ? `${pct}%` : "0%" }} />
                  </div>
                </div>
              );
            })}
        ...
        <Card className="lg:col-span-2">
          ...
          <CardContent>
            {conflicts.length === 0 ? (
              <p className="text-sm text-muted-foreground">No one is booked over capacity in the next 12 weeks.</p>
            ) : (
              <ul className="space-y-2">
                {conflicts.slice(0, 8).map((c, i) => (
                  <li key={i} className="flex items-center justify-between rounded-md border border-red-200 bg-red-50 px-3 py-2 text-sm">
                    <span>
