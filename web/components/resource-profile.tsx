...
function ProfileBody({
  ...
  const horizonWeeks = lastEnd
    ? Math.min(
        Math.max(
          Math.round(
            (startOfWeek(lastEnd).getTime() - currentMonday.getTime()) / (7 * 864e5)
          ) + 3,
          MIN_WEEKS
        ),
        MAX_WEEKS
    ...
    <div className="space-y-5">
      <SheetHeader className="space-y-2 pr-8">
        ...
        <SheetDescription className="flex flex-wrap items-center gap-x-2 gap-y-1">
          ...
          <span>·</span>
          <span>{r.dayRate ? `${formatCurrency(r.dayRate)}/day` : "no day rate"}</span>
        </SheetDescription>
      </SheetHeader>
      ...
      >
        <CalendarCheck
          className={`h-4 w-4 shrink-0 ${
            fullyFreeNow ? "text-emerald-600" : "text-muted-foreground"
          }`}
        />
        {fullyFreeNow ? (
          ...
          <span>
            <span className="font-medium">
              Fully free from {formatDate(toISO(fullyFreeDate))}.
            </span>{" "}
            {nextOpeningIdx >= 0 && (
              <span className="text-muted-foreground">
                ...
                {nextOpeningIdx === 0
                  ? "this week"
                  : `wk of ${formatWeek(weeks[nextOpeningIdx])}`}
                .
              </span>
            )}
          </span>
      ...
      <div className="grid grid-cols-3 gap-3">
        <Kpi
          label={`Avg utilization (${MIN_WEEKS} wks)`}
          value={`${Math.round(avgUtil * 100)}%`}
          tone={avgUtil > 1 ? "bad" : undefined}
        />
        <Kpi label={`Free hours (${MIN_WEEKS} wks)`} value={`${freeTotal}h`} />
        <Kpi
          label="Booked value (all work)"
          value={r.dayRate ? formatCurrency(bookedValue) : "—"}
        />
      </div>
            ...
            <Table>
              ...
              <TableBody>
                {myAllocs.map((a) => {
                  ...
                  if (!p) return null;
                  const ended = parseLocalDate(a.endDate) < currentMonday;
                  return (
                    <TableRow key={a.id} className={ended ? "opacity-50" : ""}>
                      <TableCell className="pl-6">
                        <div className="flex items-center gap-2 font-medium">
                          ...
                          <Badge
                            variant="secondary"
                            className={`px-1.5 py-0 text-[10px] ${STATUS_COLORS[p.status]}`}
                          >
                            {p.status}
                          </Badge>
                        </div>
