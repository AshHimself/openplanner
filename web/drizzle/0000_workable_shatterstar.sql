CREATE TYPE "public"."role" AS ENUM('viewer', 'planner', 'admin');--> statement-breakpoint
CREATE TYPE "public"."status" AS ENUM('Planning', 'Active', 'On Hold', 'Completed');--> statement-breakpoint
...
ALTER TABLE "timesheet_entries" ADD CONSTRAINT "timesheet_entries_resource_id_resources_id_fk" FOREIGN KEY ("resource_id") REFERENCES "public"."resources"("id") ON DELETE set null ON UPDATE no action;--> statement-breakpoint
CREATE INDEX "ts_project_week_idx" ON "timesheet_entries" USING btree ("project_id","week_of");